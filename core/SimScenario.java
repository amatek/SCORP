/* 
 * SimScenario.java was adapted to cope with SCORP.
 * 
...
public class SimScenario implements Serializable {
	...
	/** DTNHost interest for SCORP	 */
	public static final String NROF_INTERESTS = "nrofInterests";
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			/** DTNHost interest for SCORP	 */
			int nrofInt = s.getInt(NROF_INTERESTS);
			List<String> hostInterest = new ArrayList<String>();
			
			for (int a=1; a<=nrofInt; a++){
				String interest = s.getSetting(INTEREST+a);
				hostInterest.add(interest);
			}
			/** DTNHost interest for SCORP	 */
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				DTNHost host = new DTNHost(this.messageListeners, 
						this.movementListeners,	hostInterest, gid, mmNetInterfaces, comBus, 
